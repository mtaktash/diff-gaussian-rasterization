	...
	const bool debug) 
{
  ...
  torch::Tensor dcolors_dmean2D_sqr = torch::zeros({P, 2}, means3D.options());
  ...
  {  
	  ...
	  dcolors_dmean2D_sqr.contiguous().data<float>(),
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dcolors_dmean2D_sqr);
}
